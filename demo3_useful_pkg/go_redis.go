...
func standalon_redis_test() {
	...
	userinfo := Userinfo{Username: "aaron", UserID: 1, Userno: "19648"}
	userinfojson, err := json.Marshal(&userinfo)
	if err != nil {
		return
	}

	err = client.Set("userinfo1", userinfojson, 0).Err()
	if err != nil {
		fmt.Printf("try set key[userinfo1] to value struct error[%s]\n",
			err.Error())
		err_handler(err)
	}

	value, err := client.Get("userinfo1").Result()
	if err != nil {
		fmt.Printf("get key userinfo1 error[%s]\n", err.Error())
	}
	userinfobytes := []byte(value)
	userinforet := &Userinfo{}
	err = json.Unmarshal(userinfobytes, userinforet)

	fmt.Printf("key[userinfo1] values: username:%s\n", userinforet.Username)
